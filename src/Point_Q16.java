 ...
 */
public class Point_Q16 {
    int x;
    int y;

    public Point_Q16(){//constructor with no args

    }
public Point_Q16(int x, int y){//constructor with 2 args
        this.x = x;
        this.y = y;

}

    public int getX() {//instance method
        return x;
    }

    public int getY() {//instance get method
        return y;

    }

    public void setX(int x) {//instance method set method
        this.x = x;
    }

    public void setY(int y) {//instance method set method
        this.y = y;
    }
   public double distance(){//methods with parameters and return value
return Math.sqrt((0-this.getX())*(0-this.getX()) + ((0-this.getY()) * (0-this.getY())));
   }
    public double distance(int x, int y){//methods with parameters
        return Math.sqrt((x-this.getX())*(x-this.getX()) +((y-this.getY()) * (y-this.getY())));
        }
public double distance(Point_Q16 second){//methods with parameters
        return Math.sqrt((second.getX())*(second.getX()) + ((second.getY()) * (second.getY())));

//method overloading
}

    public static void main(String[] args) {//main method
        Point_Q16 first = new Point_Q16(6, 5);
        Point_Q16 second = new Point_Q16(3, 1);
        System.out.println("distance(0,0)= " + first.distance());
        System.out.println("distance(second)= " + first.distance(second));
        System.out.println("distance(2,2)= " + first.distance(2, 2));
        Point_Q16 point = new Point_Q16();
        System.out.println("distance()= " + point.distance());
    }
